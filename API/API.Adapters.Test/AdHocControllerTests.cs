...
public class AdHocControllerTests : IClassFixture<ApplicationFixture>, IAsyncLifetime
{
    ...
    public AdHocControllerTests(ApplicationFixture factory)
    {
        ...
        _seededMeldinger = TestData.CreatePostMeldingBodyFaker()
            .Generate(3)
            .Select(m => (m with
            {
                ApplicationId = ApplicationFixture.AdHocApplicationId,
                StructuredData = TestData.CreateFormFile("structuredData.json", "{ \"key\": \"value\" }", "application/json"),
                Attachments = [
                    TestData.CreateFormFile(MainContentFileName, "This should be the main document", "text/plain"),
                    TestData.CreateFormFile("some-attachment.txt", "Attachment content", "text/plain"),
    ...
    public async Task MigrateDocuments_FindMainDocument()
    {
        ...
        var myAppMeldinger = (await _client.GetFromJsonAsync<GetAllMeldingerResponse>("meldinger?pageSize=100",
            _jsonSerializerOptions,
            cancellationToken: TestContext.Current.CancellationToken))?.Items.Where(m => m.ApplicationId == ApplicationFixture.AdHocApplicationId ).ToList()!;
        ...
        {
            var documents = (await _client.GetFromJsonAsync<GetAllDocumentsResponse>(
                $"/meldinger/{m.Id}/documents",
                _jsonSerializerOptions,
                TestContext.Current.CancellationToken
            ))?.Documents!;
            
            var newMainContent = documents.First(d => d.FileMetadata.FileName == MainContentFileName).DocumentId;
            var expectedRemainingAttachments = documents.Where(d => d.DocumentId != newMainContent && d.DocumentId != m.StructuredDataId).Select(d => d.DocumentId).ToList();
            
             var newAttachments = expectedRemainingAttachments.Take(2).ToList();
            
            var httpResponse = await _client.PostAsJsonAsync(
                "/adHoc/editMelding",
                new PostEditMeldingBody()
                {
                    MeldingId = m.Id,
                    NewMainContentId = newMainContent,
                },
                _jsonSerializerOptions,
                cancellationToken: TestContext.Current.CancellationToken
            );
            ...
            var resultMelding = await httpResponse.Content.ReadFromJsonAsync<Melding>(_jsonSerializerOptions, TestContext.Current.CancellationToken);
            ...
            var docs = (await _client.GetFromJsonAsync<GetAllDocumentsResponse>(
                $"/meldinger/{m.Id}/documents",
                _jsonSerializerOptions,
                TestContext.Current.CancellationToken
            ))?.Documents!;
            ...
            await _meldingNotificationService.Received(1).NotifyMeldingProcessed(Arg.Is<Melding>(p => p.Id == resultMelding.Id));
        }
    }
