...
{
    public class Migration_AddStructuredData_Tests : IClassFixture<PostgresDbDemoFixture>, IAsyncLifetime
    {
        ...
        public async Task Migration_Sets_DocumentType_Correctly()
        {
            ...
            {
                await db.Database.MigrateAsync("20260126065135_RefactorSubscriptionModel", cancellationToken: TestContext.Current.CancellationToken);
            }
...
";
            ...
            {
                var docs = await db2.Documents.ToListAsync(cancellationToken: TestContext.Current.CancellationToken);
                docs.ShouldContain(d => d.Id == docId1 && d.DocumentType == DocumentType.StructuredData);
                docs.ShouldContain(d => d.Id == docId2 && d.DocumentType == DocumentType.Attachment);
                docs.ShouldContain(d => d.Id == docId3 && d.DocumentType == DocumentType.MainContent);
                docs.ShouldContain(d => d.Id == docId4 && d.DocumentType == DocumentType.Attachment);
                docs.ShouldContain(d => d.Id == docId5 && d.DocumentType == DocumentType.Attachment);
            }
        }
